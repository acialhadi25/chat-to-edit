...
13. PROACTIVE SUGGESTIONS: For common document fixes (tone, translate, rewrite), include a technical "action" object directly in the quickOption.
    Example quickOption: 
    { "id": "suggest-formal", "label": "Buat Lebih Formal", "value": "Applied formal tone", "isApplyAction": true, "variant": "success", "action": { "type": "TONE_ADJUST", "tone": "formal" } }
...
${preview}`;
    ...
    if (LO_KEY) {
      try {
        ...
        if (!response.ok && [402, 429, 503, 504].includes(response.status) && DS_KEY) {
          console.log(`Primary Docs AI failed (${response.status}). Attempting Deepseek fallback...`);
          usedFallback = true;
        } else if (!response.ok && DS_KEY) {
          console.log(`Primary Docs AI returned ${response.status}. Attempting Deepseek fallback anyway...`);
          usedFallback = true;
        }
      } catch (e) {
        console.error("Primary Docs AI fetch error:", e);
        if (DS_KEY) {
          usedFallback = true;
        } else {
          throw e;
    ...
    if (!response! || !response.ok) {
      ...
      console.error("AI gateway/fallback docs error:", status, errorText);
