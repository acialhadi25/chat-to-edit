...
import { MessageSquare, X, FileSpreadsheet, Save } from 'lucide-react';
...
const ExcelDashboard = () => {
  ...
  const handleFileUpload = useCallback(
    async (
      data: Omit<ExcelData, 'selectedCells' | 'pendingChanges' | 'formulas'> & {
        formulas?: Record<string, string>;
      }
    ) => {
      const uploadData = data as Omit<
        ExcelData,
        'selectedCells' | 'pendingChanges' | 'formulas'
      > & {
        formulas?: Record<string, string>;
  ...
  const handleAddColumn = useCallback(() => {
    ...
    toast({
      ...
      description: `Kolom ${getColumnLetter(newColIndex)} ditambahkan`,
    });
  }, [excelData, pushState, toast]);
  ...
  const handleApplyTemplate = useCallback(
    async (template: ExcelTemplate) => {
      ...
      const record = await saveFileRecord(template.name, template.sampleData.length, 1);
      if (record) {
        setFileHistoryId(record.id);
      }
    ...
    (newValue: string) => {
      ...
      setExcelData((prev) => {
        ...
        if (newValue.startsWith('=')) {
          const { data: withFormula } = setCellFormula(
            newData,
            parsedRef.col,
            parsedRef.row,
            newValue
          ...
          const parsedVal =
            isNaN(Number(newValue)) || newValue === '' ? newValue : Number(newValue);
          const { data: withValue } = setCellValue(
            newData,
            parsedRef.col,
            parsedRef.row,
            newValue === '' ? null : parsedVal
          );
          return withValue;
        }
      });
  ...
  const handleCellSelect = useCallback(
    (cellRefs: string[], isSelecting?: boolean) => {
      if (!excelData) return;
      setExcelData({
        ...excelData,
  ...
  const handleCellEdit = useCallback(
    (colIndex: number, rowIndex: number, newValue: string) => {
      if (!excelData) return;
      const beforeData = cloneExcelData(excelData);
      let newData = cloneExcelData(excelData);
  ...
  const handleDeleteSelection = useCallback(
    (mode: 'clear' | 'delete') => {
      ...
      const isFullRowSelection = Array.from(rowSet).every((r) =>
        Array.from({ length: excelData.headers.length }, (_, i) => createCellRef(i, r)).every(
          (ref) => selectedRefs.includes(ref)
        )
      );

      const isFullColSelection = Array.from(colSet).every((c) =>
        Array.from({ length: excelData.rows.length }, (_, i) => createCellRef(c, i)).every((ref) =>
  ...
  const handleApplyAction = useCallback(
    async (action: AIAction) => {
      ...
      const { validateExcelAction, getValidationErrorMessage, logValidationResult } =
        await import('@/utils/actionValidation');
      ...
      switch (action.type) {
          ...
          if (action.target && action.conditionType && action.formatStyle) {
            ...
            refs.forEach((ref) => {
              ...
              switch (action.conditionType) {
                case '>':
                case 'greater_than':
                  match = !isNaN(numVal) && !isNaN(condNum) && numVal > condNum;
                  break;
                case '<':
                case 'less_than':
                  match = !isNaN(numVal) && !isNaN(condNum) && numVal < condNum;
                ...
                case '=':
                case 'equal_to':
                  match = String(val) === String(condVal);
                  break;
                case '!=':
                case 'not_equal':
                  match = String(val) !== String(condVal);
                  break;
                case 'contains':
                  match = String(val).toLowerCase().includes(String(condVal).toLowerCase());
                  break;
                case 'not_contains':
                  match = !String(val).toLowerCase().includes(String(condVal).toLowerCase());
                  break;
                case 'empty':
                  match = val === null || val === undefined || String(val).trim() === '';
                  break;
                case 'not_empty':
                  match = val !== null && val !== undefined && String(val).trim() !== '';
                  break;
                case 'between': {
                  ...
                  const condNum2 =
                    typeof condVal2 === 'number' ? condVal2 : parseFloat(String(condVal2));
                  match =
                    !isNaN(numVal) &&
                    !isNaN(condNum) &&
                    !isNaN(condNum2) &&
          ...
          if (action.formula && action.target) {
            if (action.target.type === 'cell') {
              ...
              if (match) {
                ...
                const interpolatedFormula = action.formula.replace(
                  /\{row\}/g,
                  String(rowIndex + 2)
                );
                const { data: withFormula, change } = setCellFormula(
                  newData,
                  colIndex,
                  rowIndex,
                  interpolatedFormula
              ...
              const { data: withFormula, changes } = applyFormulaToColumn(
                newData,
                colIndex,
                action.formula
              );
        ...
        case 'FIND_REPLACE': {
          if (action.findValue !== undefined) {
            ...
            const { data: replacedData, changes } = findReplace(
              newData,
              action.findValue,
              action.replaceValue || '',
              { columns: targetColumns, wholeCell: false }
            );
        ...
        case 'REMOVE_EMPTY_ROWS': {
          ...
          description =
            removedRows.length > 0
              ? `Removed ${removedRows.length} empty rows (row ${rowsPreview}${moreRows})`
              : 'No empty rows found';
          break;
        }
        case 'SORT_DATA': {
        ...
        case 'DELETE_ROW': {
          if (action.target?.type === 'row') {
            ...
            action.changes.forEach((c) => {
              const match = c.cellRef.match(/\d+/);
              if (match) rowSet.add(parseInt(match[0], 10) - 2);
            });
          ...
          if (action.mergeColumns && action.mergeColumns.length > 0) {
            const { data: mergedData } = trackOperationSync(
                ...
                ),
              {
                rowCount: newData.rows.length,
                columnCount: newData.headers.length,
                columnsToMerge: action.mergeColumns.length,
              }
            );
            newData = mergedData;
...
Gunakan format JSON wajib berikut:
{
  ...
  const handleSetPendingChanges = useCallback(
    (changes: DataChange[]) => {
      if (!excelData) return;
        ...
        <div className="flex flex-1 flex-col border-r border-border min-h-0 min-w-0 overflow-hidden" style={{ touchAction: 'auto' }}>
          {!excelData ? (
            ...
            <ExcelPreview
              ...
              onSaveAsTemplate={() => setShowCreateTemplate(true)}
