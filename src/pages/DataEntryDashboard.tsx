...
const DataEntryDashboard = () => {
  ...
  const handleSmartGenerate = () => {
    ...
    setTimeout(() => {
      ...
      if (p.includes("jual") || p.includes("sales") || p.includes("dagang")) {
      ...
      } else if (p.includes("stok") || p.includes("barang") || p.includes("gudang") || p.includes("stok")) {
        fileName = "Inventaris_Gudang";
        const headers = ["Kode", "Nama Barang", "Kategori", "Stok Awal", "Masuk", "Keluar", "Sisa Stok"];
        ...
        const formulas = [
          ...
          { col: "I", formula: `=IFERROR(INDEX(F2:F6, MATCH(H{row}, A2:A6, 0)), "Tidak Ditemukan")` }
        ];
        const sampleData = [
  ...
  const handleSelectTemplate = (template: ExcelTemplate) => {
    setGeneratedPlan({
      ...
      sheets: [{
        ...
        formulas: template.formulas?.map(f => ({ col: getColumnLetter(f.column), formula: f.formula })) || []
      }]
    });
  ...
  const downloadExcel = () => {
    ...
    try {
      ...
      toast({ title: "Download Berhasil", description: `${generatedPlan.fileName}.xlsx telah diunduh.` });
    } catch (e) {
      toast({ variant: "destructive", title: "Download Gagal", description: "Terjadi kesalahan saat membuat file." });
    }
  };
  ...
  return (
    <div className="container mx-auto py-8 max-w-5xl space-y-8 animate-in fade-in duration-500">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">Apa yang ingin Anda buat hari ini?</h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Jelaskan kebutuhan Anda, AI akan membuatkan struktur Excel lengkap dengan rumus dan styling profesional.</p>
      </div>
          ...
          <div className="bg-muted/50 p-4 border-t border-border">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <Button variant="outline" size="lg" onClick={() => setShowGallery(true)} className="h-12 border-primary/20 hover:bg-primary/5 transition-all rounded-xl font-medium flex-1 sm:flex-none">
                  <FileSpreadsheet className="h-5 w-5 mr-2 text-primary" />
                  Browse Templates
                </Button>
                <Button onClick={handleSmartGenerate} disabled={isAnalyzing || !prompt} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-md h-12 px-8 rounded-xl font-semibold transition-all hover:scale-105 active:scale-95 flex-1 sm:flex-none">
                  {isAnalyzing ? <><div className="animate-spin mr-2 h-5 w-5 border-2 border-current border-t-transparent rounded-full" /> Sedang Berpikir...</> : <><Wand2 className="h-5 w-5 mr-2" /> Generate Excel</>}
                </Button>
              </div>
            </div>
          </div>
      ...
      {showGallery && <TemplateGallery onSelectTemplate={handleSelectTemplate} onClose={() => setShowGallery(false)} />}

      {!generatedPlan && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {presetPrompts.map((preset, i) => (
            <button key={i} onClick={() => setPrompt(`Buatkan ${preset.title} dengan kolom ${preset.desc}`)} className="text-left p-4 rounded-xl bg-card border border-border hover:border-primary/50 hover:shadow-md transition-all group">
              <div className="font-semibold text-foreground group-hover:text-primary flex items-center">{preset.title}<ArrowRight className="h-4 w-4 ml-auto opacity-0 group-hover:opacity-100 transition-opacity text-primary" /></div>
          ...
          <Card className="border border-border shadow-md bg-card overflow-hidden">
            <div className="bg-muted/50 p-4 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center text-green-600 dark:text-green-400"><FileSpreadsheet className="h-6 w-6" /></div>
                <div><h3 className="font-bold text-foreground">{generatedPlan.fileName}.xlsx</h3><p className="text-xs text-muted-foreground">Siap untuk diunduh</p></div>
              </div>
              <Badge className="bg-green-100 text-green-700 hover:bg-green-200 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800">Ready</Badge>
            </div>
            <CardContent className="p-0">
            ...
            <CardFooter className="p-4 bg-muted/50 flex justify-end gap-3 border-t border-border">
              <Button variant="ghost" onClick={() => setGeneratedPlan(null)}>Buat Ulang</Button>
              <Button onClick={downloadExcel} className="bg-green-600 hover:bg-green-700 text-white font-bold"><Download className="h-4 w-4 mr-2" /> Download File</Button>
            </CardFooter>
          </Card>
        </div>
      )}
