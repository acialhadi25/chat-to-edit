const CHAT_PDF_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat-pdf`;

interface StreamChatPdfParams {
  messages: { role: string; content: string }[];
  pdfContext: Record<string, unknown> | null;
  onDelta: (deltaText: string) => void;
  onDone: (fullText: string) => void;
  onError: (error: Error, status?: number) => void;
}

export async function streamChatPdf({
  messages,
  pdfContext,
  onDelta,
  onDone,
  ...
  try {
    ...
    const resp = await fetch(CHAT_PDF_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`,
      },
      body: JSON.stringify({ messages, pdfContext }),
    });

    if (!resp.ok) {
      const errorData = await resp.json().catch(() => ({}));
      const errorMsg = errorData.error || `Request failed with status ${resp.status}`;
      console.error("Chat PDF error:", { status: resp.status, error: errorMsg });
    ...
    const errorContext = `PDF Chat Streaming (URL: ${CHAT_PDF_URL})`;

    console.error("Stream Chat PDF error:", {
      message: errorMsg,
