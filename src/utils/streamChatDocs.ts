const CHAT_DOCS_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat-docs`;

interface StreamChatDocsParams {
  messages: { role: string; content: string }[];
  docsContext: Record<string, unknown> | null;
  onDelta: (deltaText: string) => void;
  onDone: (fullText: string) => void;
  onError: (error: Error, status?: number) => void;
}

export async function streamChatDocs({
  messages,
  docsContext,
  onDelta,
  onDone,
  onError,
}: StreamChatDocsParams) {
  try {
    ...
    const resp = await fetch(CHAT_DOCS_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`,
      },
      body: JSON.stringify({ messages, docsContext }),
    });

    if (!resp.ok) {
      const errorData = await resp.json().catch(() => ({}));
      const errorMsg = errorData.error || `Request failed with status ${resp.status}`;
      console.error("Chat Docs error:", { status: resp.status, error: errorMsg });
    ...
    const errorContext = `Docs Chat Streaming (URL: ${CHAT_DOCS_URL})`;

    console.error("Stream Chat Docs error:", {
      message: errorMsg,
