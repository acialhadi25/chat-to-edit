...
const FormulaBar: React.FC<FormulaBarProps> = ({
  ...
  return (
    <div className="flex items-center gap-0 w-full h-10 border-b border-border bg-card px-2 overflow-hidden shadow-sm">
      {/* Name Box */}
      <div
        className="flex items-center justify-center min-w-[70px] h-7 px-2 font-mono text-sm text-muted-foreground bg-background border border-border rounded-l select-none"
        role="status"
      ...
      {/* fx Icon */}
      <div
        className="flex items-center justify-center w-8 h-8 text-primary/70 select-none"
        aria-hidden="true"
      >
      ...
      <div className="flex-1 h-full flex items-center px-1">
        <input
          ...
          aria-label="Formula bar - Enter cell value or formula"
          role="textbox"
        />
      </div>
    </div>
