...
const DataSummaryPreview = ({ data, action }: DataSummaryPreviewProps) => {
  const summaryData = useMemo<SummaryData | null>(() => {
    ...
    if (action.type === "PIVOT_SUMMARY" && action.groupByColumn !== undefined && action.aggregateColumn !== undefined) {
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="h-8 text-[10px] uppercase">{summaryData.type === "statistics" ? "Metrik" : summaryData.groupBy}</TableHead>
