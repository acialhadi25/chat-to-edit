            ...
            <div className="space-y-1">
              {report.outliers.slice(0, 3).map((outlier, i) => (
                <div key={i} className="text-[11px] flex items-center justify-between">
                  <span>
                    Sel {outlier.cellRef}: {outlier.value}
                  </span>
              ...
              {report.outliers.length > 3 && (
                <p className="text-[10px] text-muted-foreground">
                  ...dan {report.outliers.length - 3} lainnya
                </p>
              )}
            </div>
          </Card>
          ...
          {report.suggestions.map((s, idx) => {
            ...
            return (
              <Button
                ...
                size="sm"
                disabled={isApplied}
                className={`w-full justify-between h-auto py-2 px-3 text-left border-none transition-all duration-300 ${
                  isApplied
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${
                      isApplied ? 'bg-white/20' : 'bg-white/10'
                    }`}
                  >
                    {isApplied ? (
                ...
                {!isApplied && (
                  <ArrowRight className="h-3.5 w-3.5 text-white/50 group-hover:text-white group-hover:translate-x-0.5 transition-all" />
                )}
